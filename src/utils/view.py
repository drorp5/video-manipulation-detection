...
def add_text_box(img: np.ndarray, text: str) -> np.ndarray:
    """
    Add a text box with the given text to the image.
    ...
    Returns:
        np.ndarray: The image with the added text box.
    """
    dst_img = img.copy()
    ...
    text_box_color = (
        ...
        255,
        250,
    )  # Adjust background color (white with slight transparency)
    ...
    cv2.putText(
        dst_img, text, (text_x, text_y), font, font_scale, (0, 0, 255), 1
    )  # Black text
    return dst_img
