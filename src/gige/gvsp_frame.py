...
class MockFrame:
    ...
    def packets_to_raw_image(
        self, payload_packets: PacketList
    ) -> Tuple[np.ndarray, bool]:
        raw_pixels = np.zeros(self.shape, dtype=np.uint8)
        assigned_pixels = np.zeros(self.shape, dtype=bool)

        max_payload_size_bytes = np.max(
            [len(bytes(pkt[Layers.GVSP.value].payload)) for pkt in payload_packets]
        )
        for packet in payload_packets:
            ...
            rows_indices, cols_indices = self.packet_id_to_payload_indices(
                packet_id=current_id,
                payload_size_bytes=len(pkt_bytes),
                max_payload_size_bytes=max_payload_size_bytes,
            )
...
def gvsp_frame_to_rgb(
    frame: Frame, cv2_transformation_code: int = CV2_CONVERSIONS[PixelFormat.BayerRG8]
) -> np.array:
...
def convert_image_to_rgb(img: np.ndarray, pixel_format: PixelFormat) -> np.ndarray:
