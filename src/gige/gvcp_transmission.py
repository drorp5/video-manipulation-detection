...
from .gige_constants import Layers
...
class GvcpPcapParser(PcapParser):
    ...
    def find_writereg_ack_packet(
        self, reg: GigERegisters, packet_offset: int
    ) -> Optional[int]:
        write_reg_pkt = None
        ...
        for packet_data, _ in raw_reader:
            if pkts_counter >= packet_offset:
                ...
                if pkt.haslayer(Layers.GVCP.value):
                    if (
                        pkt[Layers.GVCP.value].Command == GvcpCommands.WRITEREG_CMD
                        and pkt[Layers.GVCP.value].RegisterAddress == reg
                    ):
                        write_reg_pkt = pkt
                        write_reg_pkt_offset = pkts_counter
                    elif (
                        pkt[Layers.GVCP.value].Command == GvcpCommands.WRITEREG_ACK
                        and write_reg_pkt
                        and pkt[Layers.GVCP.value].RequestID == write_reg_pkt.RequestID
                    ):
