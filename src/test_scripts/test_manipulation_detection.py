...
def detect_in_gvsp_transmission(
    gvsp_transmission: GvspPcapParser,
    ...
    try:
        for frame in gvsp_transmission.frames:
            if frame is not None and frame.success_status:
                ...
                manipulation_detection_results = combined_detector.detect_experiments(
                    frame
                )
                failed_status = [
                    res.message
                    for res in manipulation_detection_results.values()
                    if not res.passed
                ]
                passed = len(failed_status) == 0
                manipulation_detection_scores = zip(
                    manipulation_detection_results.keys(),
                    [res.score for res in manipulation_detection_results.values()],
                )
                detection_scores.update(manipulation_detection_scores)

                fake_results = {"combined": not passed}
                detectors_fake_flag = zip(
                    manipulation_detection_results.keys(),
                    [not res.passed for res in manipulation_detection_results.values()],
                )
                ...
                if output_video_path is not None:
                    ...
                    if video_writer is None:
                        ...
                        video_writer = cv2.VideoWriter(
                            output_video_path.as_posix(), fourcc, fps, (width, height)
                        )
                    if len(vehicle_detections) > 0:
                        frame_img = draw_detections(
                            img=frame_img, detections=vehicle_detections
                        )
                    try:
                        optical_flow_img = combined_detector.image_processing_detectors[
                            -1
                        ].draw_optical_flow_tracks()
        ...
        results_df = pd.concat(
            [
                scores_df,
                fake_df.add_prefix("fake_"),
                process_time_df.add_prefix("time_"),
            ],
            axis=1,
        )
    return results_df
...
if __name__ == "__main__":
    dst_dir = Path("OUTPUT")
    gvsp_path = Path(r"..\INPUT\live_stream_defaults_start.pcapng")
    ...
    combined_detector = CombinedDetector(
        [
            constant_metadata_detector,
            frame_id_detector,
            timestamp_detector,
            timestamp_rate_detector,
        ],
        [mse_detector, histogram_detector, optical_flow_detector],
    )
    vehicle_detector = get_detector("MobileNet")
    output_video_path = None
    results_df = detect_in_gvsp_transmission(
        gvsp_transmission=gvsp_transmission,
